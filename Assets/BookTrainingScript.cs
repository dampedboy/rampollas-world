...
public class BookTrainingScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // Se il giocatore preme il tasto T e non sta già visualizzando immagini
            if (Input.GetKeyDown(KeyCode.T) && !isViewingImages)
            {
                StartViewingImages();
            }
            // Se il giocatore preme il tasto T e sta visualizzando immagini
            else if (Input.GetKeyDown(KeyCode.T) && isViewingImages)
            {
                ShowNextImage();
            }
            // Se il giocatore preme il tasto Y per uscire dalla visualizzazione delle immagini
            else if (Input.GetKeyDown(KeyCode.Y) && isViewingImages)
            {
                ExitViewingImages();
            }
        }
